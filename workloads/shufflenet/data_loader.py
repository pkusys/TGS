...
class DataLoader:
    ...
    def load_data(self):
        ...
        train_data = np.array([plt.imread('/cluster/workloads/shufflenet/data/0.jpg') for _ in range(self.batch_size)], dtype=np.float32)
        self.X_train = train_data
        self.y_train = np.array([283 for _ in range(self.batch_size)], dtype=np.int32)

        val_data = np.array([plt.imread('/cluster/workloads/shufflenet/data/0.jpg')], dtype=np.float32)
        self.X_val = val_data
