...
def benchmark_gpt_bert_one_case(benchmark_case, output_file_name, _gbs, _repeat,
    _scheduler_ip, _scheduler_port, _trainer_port, _job_id):
    # Model configs
    model_type = "gpt"
    (model_name, global_batch_size, model_config, num_micro_batches, parallel_mode,
    ...
    if _gbs != -1 and gbs % (dp_size * num_micro_batches) == 0:
    ...
    if rank == 0:
        trainer = Trainer(_scheduler_ip, _scheduler_port, utils.get_host_ip(), _trainer_port, _job_id, global_batch_size)

    # Check initialization
    # print('------', dp_size, '-', mpu.get_data_parallel_world_size(), '------')
    ...
    def run_func():
        ...
        run_func.cnt += 1
        if rank == 0:
            trainer.record(interval)
        print('---Rank ' + str(rank) + ", " + str(run_func.cnt) +
                " th step, normalized throughput: " + str(1000/interval) + '---')
    ...
    if rank == 0:
        ...
        print("Sleeping for 1 second before starting the next case. ")
        time.sleep(1)
...
if __name__ == "__main__":
    ...
    scheduler_ip = configs['scheduler_ip']
    scheduler_port = configs['scheduler_port']
    trainer_port = configs['trainer_port']
    job_id = configs['job_id']

    case = eval(sys.argv[-2])
    output_file_name = sys.argv[-1]
    del sys.argv[-1]
    del sys.argv[-1]
    benchmark_gpt_bert_one_case(case, output_file_name, gbs, repeat,
            scheduler_ip, scheduler_port, trainer_port, job_id)
